...
elem <- remDr$findElement(using = 'xpath', "//*/option[@value = 'Masters']")
elem$clickElement()
...
```
```{r}
save(scores, teams, file=paste0("MastersWomensScores",format(Sys.time(),"%Y %m %d"),".Rdata"))
```
...
```{r}
write.csv(rankedteams, paste("USAU Masters Women RW", format(Sys.time(),"%Y %m %d"),".csv",sep=""), row.names = TRUE)

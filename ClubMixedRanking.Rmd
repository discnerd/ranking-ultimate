...
```
```{r}
save(scores, teams, file=paste0("ClubMixedScores",format(Sys.time(),"%Y %m %d"),".Rdata"))
```
...
```{r}
write.csv(rankedteams, paste("USAU Club Mixed RW", format(Sys.time(),"%Y %m %d"),".csv",sep=""), row.names = TRUE)

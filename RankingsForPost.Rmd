...
mensRanking <- left_join(mensRanking,oldmensRanking,by=c("Links")) %>% mutate(Ranking.diff = Ranking.y-Ranking.x, Rating.diff=Rating.y-Rating.x) %>% select(Ranking.x, Ranking.diff, Rating.diff, Team.x, contains(".x")) %>% select(Ranking.x, Team.x, Ranking.diff, Rating.x, Rating.diff, `Competition Division.x`:Losses.x) %>% rename(Team=Team.x, Ranking=Ranking.x, Rating=Rating.x, `Competition Division`=`Competition Division.x`, `College Region`=`College Region.x`, `College Conference`=`College Conference.x`, Wins=Wins.x, Losses=Losses.x)

womensRanking <- left_join(womensRanking,oldwomensRanking,by=c( "Links")) %>% mutate(Ranking.diff = Ranking.y-Ranking.x, Rating.diff=Rating.y-Rating.x) %>% select(Ranking.x, Ranking.diff, Rating.diff, Team.x, contains(".x")) %>% select(Ranking.x, Team.x, Ranking.diff, Rating.x, Rating.diff, `Competition Division.x`:Losses.x) %>% rename(Team=Team.x, Ranking=Ranking.x, Rating=Rating.x, `Competition Division`=`Competition Division.x`, `College Region`=`College Region.x`, `College Conference`=`College Conference.x`, Wins=Wins.x, Losses=Losses.x) 
```

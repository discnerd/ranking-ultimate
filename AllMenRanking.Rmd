...
```{r setup}
library(RSelenium)
rD <- rsDriver(browser = "chrome", verbose = TRUE, check = FALSE, chromever = "77.0.3865.40")
remDr <- rD[["client"]]
```
```{r pullTeams}
library(tidyverse)
...
for( i in 1:length(teams$Team)){
  ...
  #pull table of games
  elem <- tryCatch({
    remDr$findElement(using = "id", value="CT_Right_0_gvEventScheduleScores")
  }, error = function(err){
    print("How do they have no scores?")
    return(NA)
  }
  )
  if(isS4(elem)){
    ...
    data <- data %>% filter(Date != Scores) %>% filter(Opponent != "N/A")
    data$Links <- elem$getElementAttribute("outerHTML")[[1]] %>%
      htmlTreeParse(useInternalNodes=TRUE) %>% 
      xpathApply("//a", xmlGetAttr, 'href') %>% 
      unlist %>% grep("Eventteam",., value=TRUE) %>% .[1:length(data$Opponent)]
    
    data <- data %>% separate(Scores, c("For", "Against"))
    data <- data %>% filter(!is.na(Against) & !is.na(For))  %>%
...
n %v% "rank" <- left_join(clubTeams, Ranking, by = c("Team","State") ) %>%
  .$Rank
n %v% "rating" <- left_join(clubTeams, Ranking, by = c("Team", "State") ) %>%
  .$Rating
add.edges(n, 

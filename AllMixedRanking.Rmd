...
```{r setup}
library(RSelenium)
rD <- rsDriver(chromever = "77.0.3865.40")
remDr <- rD[["client"]]
```
```{r pullTeams}
library(tidyverse)
...
ggplot(net, aes(x = x, y = y, xend = xend, yend = yend))+
  geom_edges(alpha=0.1, arrow = arrow(length = unit(0.5, "lines"), type = "closed"))+
  geom_nodes( aes(color=rating), alpha=0.25 ) +theme_blank()+

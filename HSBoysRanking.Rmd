...
elem <- remDr$findElement(using = 'xpath', "//*/option[@value = '20']")
elem$clickElement()
...
rm(data)
rm(elem)
teams <- teams %>% filter( Wins !=0 | Losses !=0) %>% filter(!grepl("esting", Team))
...
```
```{r}
save(scores, teams, file=paste0("HgSBoysScores",format(Sys.time(),"%Y %m %d"),".Rdata"))
...
write.csv(rankedteams, paste("USAU HS Boys RW", format(Sys.time(),"%Y %m %d"),".csv",sep=""), row.names = TRUE)

...
elem <- remDr$findElement(using = 'xpath', "//*/option[@value = 'College']")
elem$clickElement()
...
teams <- teams %>% arrange(Team) %>% mutate(Wins=0, Losses=0)  %>% filter( !grepl("[Tt]esting",Team))
...
#  for( i in 457:length(teams$Team)){
...
```
```{r}
save(scores, teams, file=paste0("Data/CollegeMensScores",format(Sys.time(),"%Y %m %d"),".Rdata"))
...
write.csv(rankedteams, paste("Rankings/USAU College Men RW", format(Sys.time(),"%Y %m %d"),".csv",sep=""), row.names = TRUE)

...
elem <- remDr$findElement(using = 'xpath', "//*/option[@value = 'High School']")
elem$clickElement()

elem <- remDr$findElement(using = 'xpath', "//*/option[@value = '19']")
elem$clickElement()
...
rm(data)
rm(elem)
teams <- teams %>% filter( Wins !=0 | Losses !=0) %>% filter(!grepl("esting", Team)) %>% mutate(Wins=0, Losses=0)
...
```
```{r}
save(scores, teams, file=paste0("HSGirlsScores",format(Sys.time(),"%Y %m %d"),".Rdata"))
...
write.csv(rankedteams, paste("USAU HS Girls RW", format(Sys.time(),"%Y %m %d"),".csv",sep=""), row.names = TRUE)

```

```{r results="asis"}

rankedteams %>% select(Team, Rating, Wins, Losses, City, State) %>% head(25)  %>% knitr::kable(type="html")
